...
UI direction → skill `found-this-design` then `impeccable`. Motion → `emil-design-eng`. Media → `visual-studio`. Scroll/3D → `scroll-world`. Registry → shadcn MCP. Design Intelligence is internal to Impeccable `new-work`, never a route.
