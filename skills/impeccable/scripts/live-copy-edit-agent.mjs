...
function runClaude(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  // Do not inject foreign CLI auth tokens. OpenCode live copy-edit prefers the chat agent.
...
export function describeNoProviderError({
  ...
  if (exists('claude')) {
    lines.push('  • Claude CLI: present on PATH (foreign). OpenCodeBestFriend does not configure it. Prefer the Impeccable live chat agent.');
  } else {
